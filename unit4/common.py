...
def reinforce(env, policy, optimizer, n_training_episodes, max_t, gamma, print_every):
    # max_t ; maximum sequence of trajectory
...
def record_video(env, policy, out_directory, fps=30):
    ...
    while not (terminated or truncated):
        ...
        img = env.render()
        images.append(img)
        print(f"timestamp {timestamp}: action completed")
        timestamp += 1

    np_array_imgs = []
    while images:
        img = images.pop(0)
        np_array_imgs.append(np.array(img))

    imageio.mimsave(out_directory, np_array_imgs, fps=fps)
